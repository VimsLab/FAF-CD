...
def _strip_train_only_keys(state_dict):
    """
    Remove checkpoint entries that belong to training-only criterion modules.
    Evaluation builds `EncoderDecoder(..., criterion=None)`, so these keys are
    expected to be absent in the eval model.
    """
    train_only_prefixes = ('criterion.', 'module.criterion.')
    removed_keys = [k for k in state_dict.keys() if k.startswith(train_only_prefixes)]
    if not removed_keys:
        return state_dict

    logger.warning(
        "Ignoring %d training-only checkpoint key(s) during load. Example: %s",
        len(removed_keys),
        ', '.join(removed_keys[:3]) + (' ...' if len(removed_keys) > 3 else ''),
    )
    return OrderedDict((k, v) for k, v in state_dict.items() if k not in removed_keys)
...
def load_restore_model(model, model_file):
    ...
    state_dict = _strip_train_only_keys(state_dict)
...
def load_model(model, model_file, is_restore=False):
    ...
    if model_file is None:
        print("no model file loaded. Default vmamba weights")
        return model
...
def parse_devices(input_devices):
    if isinstance(input_devices, (list, tuple)):
        devices = [int(d) for d in input_devices]
        logger.info('using devices {}'.format(
            ', '.join([str(d) for d in devices])))
        return devices

    input_devices = str(input_devices).strip()
    if input_devices.startswith('[') and input_devices.endswith(']'):
        input_devices = input_devices[1:-1]
