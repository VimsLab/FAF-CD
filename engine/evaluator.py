...
class Evaluator(object):
    def __init__(self, dataset, class_num, norm_mean, norm_std, network, multi_scales, 
        ...
        self.config = config
        self.eval_legacy_exp_scores = bool(
            getattr(config, 'eval_legacy_exp_scores', False)
        ) if config is not None else False

        # print("Evaluator initialized with config: {}".format(config.root_dir))

    @staticmethod
    def _parse_eval_output(eval_output):
        if not isinstance(eval_output, tuple):
            raise ValueError(
                f"Evaluation output must be a tuple, got {type(eval_output).__name__}"
            )
        if len(eval_output) < 2:
            raise ValueError(
                f"Evaluation output must have at least 2 values, got {len(eval_output)}"
            )
        result_line = eval_output[0]
        mean_iou = eval_output[1]
        extra = eval_output[2:]
        return result_line, mean_iou, extra
    ...
    def run(self, model_path, model_indice, log_file, log_file_link):
        ...
        should_write_header = (not os.path.exists(log_file)) or (os.path.getsize(log_file) == 0)
        results = open(log_file, 'a')
        link_file(log_file, log_file_link)

        if should_write_header and self.config is not None:
            config_dict = {}
            ...
            results.write("=" * 80 + "\n")
            results.write("CONFIGURATION\n")
            results.write("=" * 80 + "\n")
            for key in sorted(config_dict.keys()):
                results.write(f"{key}: {config_dict[key]}\n")
            results.write("=" * 80 + "\n\n")
        ...
        for model in models:
            ...
            if len(self.devices ) == 1:
                 eval_output = self.single_process_evalutation()
            else:
                 eval_output = self.multi_process_evaluation()
            result_line, mean_IoU, extra = self._parse_eval_output(eval_output)
            dice = extra[0] if len(extra) > 0 else None
            metrics = extra[1] if len(extra) > 1 else None
        ...
        return result_line, mean_IoU, dice, metrics
    ...
    def run_eval(self, model_path, model_indice, log_file, log_file_link):
        """There are four evaluation modes:
            ...
            4.eval all epochs from a certain started epoch: -e start_epoch-
            """
        if model_indice.startswith("epoch-"):
            model_indice = model_indice.replace("epoch-", "", 1)
    ...
    def run_eval_during_train(self, model_path, model_indice, log_file, log_file_link):
        ...
        for model in models:
            ...
            if len(self.devices ) == 1:
                ...
                eval_output = self.multi_process_evaluation()
            result_line, mean_IoU, _ = self._parse_eval_output(eval_output)
    ...
    def single_process_evalutation(self):
        ...
        eval_output = self.compute_metric(all_results)
        result_line, mean_IoU, extra = self._parse_eval_output(eval_output)
        logger.info(
    ...
    def sliding_eval_rgbX(self, img, modal_x, crop_size, stride_rate, device=None, return_scores=False):
        ...
        if return_scores:
            from utils.metric import softmax_scores
            scores = softmax_scores(processed_pred)
            return pred, scores
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():
                ...
                if self.is_flip:
                    ...
                    score_flip = score_flip[0]
                    score += score_flip.flip(-1)
                if self.eval_legacy_exp_scores:
                    score = torch.exp(score)
    ...
    def process_image_rgbX(self, img, modal_x, crop_size=None):
        ...
        p_img = normalize(p_img, self.norm_mean, self.norm_std)
        if len(modal_x.shape) == 2 or (len(modal_x.shape) == 3 and modal_x.shape[2] == 1):
            mean_src = getattr(self.config, "norm_mean_B", self.norm_mean) if self.config is not None else self.norm_mean
            std_src = getattr(self.config, "norm_std_B", self.norm_std) if self.config is not None else self.norm_std
            mean_x = np.array([np.asarray(mean_src).reshape(-1)[0]])
            std_x = np.array([np.asarray(std_src).reshape(-1)[0]])
            p_modal_x = normalize(p_modal_x, mean_x, std_x)
        else:
